...
#' }
#'
create_densities = function(x, y, size){
  ...
  out = if (requireNamespace("pbapply", quietly = TRUE)){
    raster::stack(pbapply::pblapply(raster::as.list(x), create_density, y = y, size = size))
  } else {
    raster::stack(lapply(raster::as.list(x), create_density, y = y, size = size))
  }
  return(out)
}
...
# output
# plot(output)
create_density = function(x, y, size){
  # x = real_raster; y = perc_raster; size = 10
  # x = cats; y = s; size = 2;
  w = calculate_density(x, y, size = size)
  vals = motifel_to_grid(raster::as.matrix(x[[1]]), w, size = size)
